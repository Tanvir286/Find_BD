...
const AllProperty = () => {
  ...
  const handleForm = (event) => {
    ...
    fetch(
      `https://rent-server-ten.vercel.app/productCollection?price=${price}&city=${city}&rentType=${rentCheckValue}&bedAmount=${bedCheckValue}&washAmount=${washCheckValue}&month=${month}`
    )
      .then((res) => res.json())
  ...
  const handleSearch = (event) => {
    ...
    fetch(
      `https://rent-server-ten.vercel.app/sortProducts?city=${city}&area=${area}&rent=${rent}`
    )
      .then((res) => res.json())
      .then((data) => setPosts(data));
  };
  useEffect(() => {
    if (homeSearch?.city) {
      ...
      fetch(
        `https://rent-server-ten.vercel.app/sortProducts?city=${homeSearch?.city}&area=${homeSearch?.area}&rent=${homeSearch?.rent}`
      )
        .then((res) => res.json())
        .then((data) => setPosts(data));
    } else {
      const fetchPosts = async () => {
        setLoading(true);
        const res = await axios.get("https://rent-server-ten.vercel.app/productCollection");
        setPosts(res.data);
        setLoading(false);
      };
